...
{	
	...
	for(int i=0; i<100 ;i++)
	{
		if(theta<thetamax)
		{
			...
			"norm=4,\n"
			"xnorm1="<<(lb4-(1.5*r)-0.5*stem)<<",ynorm1="<<a<<",\n"
			"xnorm2="<<(lb4-(1.5*r)-0.5*stem)<<",ynorm2="<<b<<",\n"
			"enorm=1000000.0, \n"
			"IRTYPE=1,\n"
			"TEMPK=2,\n"
			"beta=0.046&          \n"
			"&po x= 0.00,  y= "<<(a+base*tan(theta))<<" & \n"
			"&po x= 0.00,  y="<<b<<",nt=5,radius="<<0.5*(b-(a+base*tan(theta)))<<"&\n"
			"&po x= "<<(lb4+gap-0.02*lb4)<<",  y= "<<b<<" &\n"
			"&po x="<<lb4+gap<<",y="<<b-(0.02*lb4)<<" ,nt=5, radius="<<(0.02*lb4)<<" &\n"
			"&po x= "<<lb4+gap<<",  y= 0.00 &\n"
			"&po x= "<<lb4<<",  y= 0.00 &\n"
			"&po x= "<<lb4<<",  y= "<<a+r<<"&\n"
			"&po x="<<(lb4-2*r)<<",y="<<a+r<<" ,nt=4, radius="<<r<<" &\n"
			"&po x="<<(lb4-3*r)<<",y="<<a<<" ,nt=5, radius="<<r<<" &\n"
			"&po x= "<<(lb4-stem)<<",  y= "<<a<<"&\n"
			"&po x= 0.00,  y= "<<( a+base*tan(theta) )<<",nt=5,radius="<<10*lb4<<" & \n";
		file.close();	
		}
